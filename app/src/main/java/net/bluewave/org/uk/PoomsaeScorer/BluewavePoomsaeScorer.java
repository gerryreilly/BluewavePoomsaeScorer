package net.bluewave.org.uk.PoomsaeScorer;
...
public class BluewavePoomsaeScorer extends Activity implements INetClientDelegate {
    public final DecimalFormat formatter = new DecimalFormat("#.#");

    protected NetClient client;
    protected EditText host, port, passphrase, editText;
    protected TextView name, ring, judge, technical, presentation, elapsedTime, poomsaeName;
    protected LinearLayout connected, disconnected, center;
    protected Button connect, confirm, minorDeduction, majorDeduction, undoMinor, undoMajor;
    ...
    protected long[] pattern1 = {0, 200, 200, 200};
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            this.elapsedTime = (TextView) this.findViewById(R.id.timePoomsae);
            this.confirm = (Button) this.findViewById(R.id.confirm);
            this.poomsaeName = (TextView) this.findViewById(R.id.poomsaeName);
            this.confirm.setVisibility(View.INVISIBLE);
        ...
        final BluewavePoomsaeScorer me = this;
    ...
    public void sendConfirm() {
        this.client.send("confirmed",this.editText.getText().toString());
        disable_confirm();
        disable_accuracy();
    ...
    public void onReceive(String[] message) {
        ...
        if (message.length >= 2 && message[0].startsWith("registered")) {
            ...
            this.enable_confirm();
         } else if (message.length == 1 && message[0].startsWith("enableAccuracy")) {
              this.enable_accuracy();
         } else if (message.length == 1 && message[0].startsWith("disableAccuracy")) {
              this.disable_accuracy();
         } else if (message.length == 1 && message[0].startsWith("enablePresentation")) {
              this.enable_presentation();
              this.enable_confirm();
        } else if (message.length == 1 && message[0].startsWith("disablePresentation")) {
                this.disable_presentation();
            ...
            this.updateInfo();
        } else if (message.length >= 2 && message[0].startsWith("elapsedTime")) {
            this.elapsedTime.setText(message[1]);
            ...
            this.updateInfo();
        } else if (message.length == 3 && message[0].startsWith("poomsae")) {
            this.poomsaes = message[1];
            this.poomsaeName.setText(message[2]);
            this.updateInfo();
    ...
    private void enable_accuracy() {
        this.majorDeduction.setEnabled(true);
        this.minorDeduction.setEnabled(true);
        this.undoMajor.setEnabled(true);
        this.undoMinor.setEnabled(true);
    }

    private void disable_accuracy() {
        this.majorDeduction.setEnabled(false);
        this.minorDeduction.setEnabled(false);
        this.undoMajor.setEnabled(false);
        this.undoMinor.setEnabled(false);
    }

    private void enable_presentation() {
        for (ScoreScale scale : this.scales) {
           scale.bar.setClickable(true);
           // scale.setVisibility(View.VISIBLE);
            scale.bar.setEnabled(true);
        }
    }

    private void disable_presentation() {
        for (ScoreScale scale : this.scales) {
            scale.bar.setClickable(false);
            scale.bar.setEnabled(false);
            //scale.setVisibility(View.INVISIBLE);
        }
    }

    private void enable_confirm() {
        this.confirm.setVisibility(View.VISIBLE);
        this.confirm.setClickable(true);
        this.confirm.setEnabled(true);
    }

    private void disable_confirm() {
        //this.confirm.setVisibility(View.INVISIBLE);
